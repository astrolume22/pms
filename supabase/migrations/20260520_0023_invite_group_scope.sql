-- =====================================================================
-- Update create_invite + accept_invite to support group-scoped invites
-- and to enforce the new "admin-only mint, manager-only role" rule.
--
-- New behaviour:
--   * create_invite accepts an optional p_group_id; only an admin can
--     mint invites (this file removes the legacy board-owner/manager
--     escape hatches per the permissions redesign).
--   * Invites can ONLY assign the 'manager' role. Admin role is never
--     mintable via invite.
--   * accept_invite carries the invite's group_id onto the new
--     board_subscribers row, giving the new manager the right scope.
-- =====================================================================

-- ---------------------------------------------------------------------
-- create_invite — admin only, manager role only, optional group scope.
-- ---------------------------------------------------------------------
...
begin
  ...
  if p_role <> 'manager' then
    raise exception 'Invites can only assign the manager role' using errcode='22023';
  end if;
  -- Group scope requires a board.
  ...
  values (v_token, 'manager', p_board_id, p_group_id, auth.uid(), v_expires_at)
...
-- Keep the old 3-arg signature working (any existing callers default
-- group_id to null) by leaving it in place. We just shadow it with the
-- 4-arg version via supabase's positional + named arg resolution.
-- (Supabase RPC sends named args, so create_invite(p_role,p_board_id,
-- p_expires_in_hours) still works if we keep both function bodies.)
-- For cleanliness, drop the 3-arg version since the 4-arg one with the
-- default covers it.
drop function if exists public.create_invite(text, uuid, int);

-- ---------------------------------------------------------------------
-- get_invite_by_token — also return group_id + group_name when scoped.
-- ---------------------------------------------------------------------
...
begin
  ...
  if v_invite.used_at  is not null then return jsonb_build_object('valid', false, 'reason', 'used'); end if;
  if v_invite.expires_at <= now() then return jsonb_build_object('valid', false, 'reason', 'expired'); end if;
...
-- ---------------------------------------------------------------------
-- accept_invite — carry the group_id onto the new subscription so the
-- RLS sees the scope. Otherwise unchanged.
-- ---------------------------------------------------------------------
...
begin
  ...
  if v_invite.expires_at <= now()    then raise exception 'This invite has expired' using errcode='42501'; end if;
  ...
  -- Board / group scope — carry both onto the new subscriber row.
  if v_invite.board_id is not null then
